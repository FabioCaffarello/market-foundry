...
| **H-8.b** | **Atual** (esta entrega — branch `feat/h-8-b-tpo`; loop autônomo, 2026-06-13) | TPO profile (Time-Price Opportunity) — segunda capacidade de insights. **Timeframe-anchored** (T1 — não session-anchored; foundry sem conceito de sessão) + **trades-only** (T2 — períodos derivados de trades, não candles). Janela de timeframe subdividida em períodos (letras A–X, cap 24 — T3); cada trade marca seu nível de preço (`BucketLevel`) com a letra do período. `TPOProfile{Periods[], Levels[]}`, `TPOLevel{PriceLevel, Letters, Count}`; POC/VAH/VAL/IB/range no snapshot (T4). Persistência **Array-columns** `insights_tpo` (períodos + níveis paralelos — T5, padrão H-8.a.1); reusa domínio insights + layer codegen `insights` + stream `INSIGHTS_EVENTS`; overload por contagem de níveis. Read `GET /insights/tpo/latest`. Decisões T1–T5 (agente, pré-flight) no [PROGRAM-0005](programs/PROGRAM-0005-insights.md). |
