...
Current state (post-Onda H-6.c.2, 2026-05-27):
...
- **Proposed** (T3, `Partially Implemented`): ADR-0021 — domain
  ...
  (H-6.c.2 commit 3). Promotion to `Accepted` remains gated on
  criterion #2 (zero source-string-based reconstruction in
  production code) — 1 executionclient helper + 13 ClickHouse
  reconstruction callers remain. Promotion is an atomic event
...
## Summary counts (2026-05-27, post-H-6.c.2)
