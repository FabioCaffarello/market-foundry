...
| Delivery WebSocket — push read-only de insights events | [ADR-0028](decisions/0028-delivery-websocket-protocol.md), [PROGRAM-0006](programs/PROGRAM-0006-delivery.md) | Domínio `internal/domain/delivery/` (`Session`, `Subscription`, `SubjectMatches` — matcher de subject NATS puro: `*`=1 token, `>`=tail terminal); port `internal/application/ports/delivery.go` (`DeliveryConn`/`DeliverySession`/`DeliveryHub` — mantém `interfaces/` sem importar `actors/`, ADR-0005); consumer durável `deliver-insights` `internal/adapters/nats/natsdelivery/consumer.go` (desde H-11.b lê `insights.events.>` — TODAS as famílias; decode dispatched por subject CBOR→JSON; frame de fio `{subject,event}` p/ o cliente demuxar; reusa `natsinsights.DefaultRegistry`); fan-out `internal/actors/scopes/delivery/` (`RouterActor` broadcast + `SessionActor` dono da `Session` + buffer outbound bounded com **política configurável** I4 — `domain.BackpressurePolicy` DropNewest default + DropOldest, H-11.c; PriorityDrop deferido; `delivery.Config`/`ConfigFromEnv`; write goroutine; `Hub`/`SessionHandle`; `Runtime.Start`); métricas Prometheus `marketfoundry_delivery_frames_total{outcome}` + `…_sessions` (gauge) + `…_sessions_rejected_total` (H-11.e) em `internal/shared/metrics`; **max-sessions cap** no `Hub` (`Config.MaxSessions`, env `MARKETFOUNDRY_DELIVERY_MAX_SESSIONS`, default 1024 — bound de subsistema do total de sessões, ADR-0028 I4, H-11.e); endpoint `GET /ws` `internal/interfaces/http/handlers/delivery.go` (gorilla upgrade + control frames JSON `{action,subject}` subscribe/unsubscribe — único inbound, I1; loopback I2) + `routes/delivery.go` (`DeliveryFamilyDeps`); wiring `cmd/gateway/run.go` (`delivery.Start` no engine; degrada se NATS off). **snapshot-then-delta (H-11.f)**: `ports.SnapshotProvider` + `natsdelivery.KVSnapshotProvider` (`parseInsightsSubject` subject→KV-key + dispatch p/ `natsinsights.Gateway`); `SessionActor.onSubscribe` envia o snapshot KV-latest antes dos deltas p/ subjects totalmente especificados (`Config.SnapshotProvider`, nil=off; wildcards só deltas). Analyzer `tools/raccoon-cli/src/analyzers/check_delivery.rs` + `policies/delivery.toml` (P5, H-11.d — gate Step 12b: reader-only + stream-bound). Single-writer (ADR-0008): delivery é **leitor**, derive segue único publisher. | `internal/domain/delivery/delivery_test.go` (matcher literal/`*`/`>`/combos + validação de Subscription + ciclo de vida de Session + cópia defensiva); `internal/actors/scopes/delivery/delivery_actors_test.go` (DropNewest determinístico + fan-out com filtro de subscrição, engine real + WSConn fake); `cmd/gateway/boot_test.go` entry `/ws`; `internal/adapters/nats/natsdelivery/consumer_test.go` (decode CBOR→JSON por família VP/TPO/cross-venue); canários integration `internal/actors/scopes/delivery/delivery_ws_canary_test.go` + `…/delivery_ws_multi_canary_test.go` (`//go:build integration` — VP, TPO, cross-venue, e multi-família/1-sessão vs NATS vivo; subject único por run evita flood do histórico); `internal/domain/delivery/backpressure_test.go` + `…/backpressure_actor_test.go` (Parse/round-trip + DropOldest evicta oldest determinístico); `…/hub_maxsessions_test.go` (cap rejeita o N+1 + double-Close idempotente + ilimitado); `natsdelivery/snapshot_test.go` (parser + provider VP) + `…/delivery/snapshot_on_subscribe_test.go` (snapshot no subscribe / nil-provider) + canário `…/delivery_snapshot_canary_test.go` (KV seed → snapshot → delta ao vivo, distinguidos por open_time); `cargo test analyzers::check_delivery` (6 tests) + gate Step 12b `check delivery` (4 checks) em `make verify`; drift-detect `insights-contracts-drift` (durable `deliver-insights`). | Implemented (H-11.a + H-11.b + H-11.c + H-11.d + H-11.e + H-11.f) | **ADR-0028 `Accepted` (2026-06-13, H-11.a)** — read-only transport (I1), loopback (I2), escopo decision-support/insights (I3 — todas as famílias desde H-11.b), backpressure bounded **configurável** DropNewest/DropOldest (I4, H-11.c; PriorityDrop deferido — insights equi-advisory, ADR-0027) + **max-sessions cap** de subsistema (I4, H-11.e), single-writer respeitado (I5). Frame de fio `{subject,event}` (H-11.b). Métricas Prometheus de delivery (H-11.c/e). Enforcement estático `check delivery` (P5, H-11.d). snapshot-then-delta no subscribe p/ subjects totalmente especificados (H-11.f). **PROGRAM-0006 Closed (Fase Delivery completa, H-11.a–f).** |
