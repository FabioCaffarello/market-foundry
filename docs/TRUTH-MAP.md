...
| Raccoon-cli `check instruments` analyzer (adapter normalization + domain-type migration state) | [ADR-0021](decisions/0021-canonical-instrument-and-venue-model.md), [PROGRAM-0004](programs/PROGRAM-0004-multi-venue.md) | `tools/raccoon-cli/src/analyzers/check_instruments.rs:analyze`; `tools/raccoon-cli/policies/adapters.toml` (adapter allowlist: `["binances", "binancef"]`); `tools/raccoon-cli/policies/domain_types.toml` (domain-type migration state, H-6.b extension); CLI variant + dispatch + gate Step 9 integration. | `cargo test analyzers::check_instruments` (14 tests covering adapter + domain-type checks). `make verify` GREEN includes `check-instruments` PASS (6 checks: adapters-dir, policy-present, adapter-allowlisted, adapter-uses-canonical-constructor, domain-types-policy-present, domain-type-migration-state). | Implemented | Declarative two-policy enforcement: adapters.toml gates new adapters; domain_types.toml gates per-type migration state. For each type marked `migrated`, the analyzer enforces `instrument.CanonicalInstrument` field reference + `VenueSymbol() string` method. `pending` types are tolerated. Both H-6.a and H-6.b enforcement layers are surfaced; H-6.f will sunset the VenueSymbol checks when accessors are removed. |
