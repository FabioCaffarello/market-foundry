...
Current state (post-Onda H-6.c.1, 2026-05-27):
...
- **Proposed** (T3, `Partially Implemented`): ADR-0021 — domain
  ...
  migration for derive scope landed in H-6.c.1 —
  `instrumentFromBinding` helper eliminated from 4 packages
  (signal/decision/strategy/risk), new `BindingTarget.Instrument()`
  boundary helper (error-returning, no silent zero) wired through
  derive actor cascade, 2 application packages remaining
  (execution → H-6.c.2; executionclient → H-6.f). Promotion to
  `Accepted` remains gated on criterion #2 (all domain-layer
  call sites migrated). Promotion is an atomic event in H-6.f
  when all criteria are literally true.
...
## Summary counts (2026-05-27, post-H-6.c.1)
