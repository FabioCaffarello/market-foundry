...
func newGatewayConn[T any](config settings.AppConfig, label string, build func(*natskit.NATSRequestClient) T) (T, func() error, *problem.Problem) {
	var zero T
	if !config.NATS.Enabled {
		return zero, nil, nil
	}

	requestClient, err := natskit.NewNATSRequestClientWithURL(config.NATS.URL, config.NATS.RequestTimeoutDuration())
	if err != nil {
