...
func declareWriterPipelines(chClient *adapterch.Client) []writerPipeline {
	...
	return []writerPipeline{
		...
		// ── Evidence: candle → evidence_candles ──
		{
			...
			insertSQL:     "INSERT INTO evidence_candles (event_id, occurred_at, correlation_id, causation_id, source, symbol, base, quote, contract, timeframe, open, high, low, close, volume, trade_count, open_time, close_time, final)",
		...
		// ── Signal: rsi → signals ───────────────────────────────
		{
			...
			table:         "signals",
			insertSQL:     "INSERT INTO signals (event_id, occurred_at, correlation_id, causation_id, type, source, symbol, base, quote, contract, timeframe, value, metadata, final, timestamp)",
		...
		// ── Decision: rsi_oversold → decisions ──
		{
			...
			table:         "decisions",
			insertSQL:     "INSERT INTO decisions (event_id, occurred_at, correlation_id, causation_id, type, source, symbol, base, quote, contract, timeframe, outcome, confidence, severity, rationale, signals, metadata, final, timestamp)",
		...
		// ── Strategy: mean_reversion_entry → strategies ──
		{
			...
			table:         "strategies",
			insertSQL:     "INSERT INTO strategies (event_id, occurred_at, correlation_id, causation_id, type, source, symbol, base, quote, contract, timeframe, direction, confidence, decisions, parameters, metadata, final, timestamp)",
		...
		// ── Risk: position_exposure → risk_assessments ──
		{
			...
			insertSQL:     "INSERT INTO risk_assessments (event_id, occurred_at, correlation_id, causation_id, type, source, symbol, base, quote, contract, timeframe, disposition, confidence, strategies, constraints, rationale, parameters, metadata, final, timestamp)",
		...
		// ── Execution: paper_order → executions ──
		{
			...
			table:         "executions",
			insertSQL:     "INSERT INTO executions (event_id, occurred_at, correlation_id, causation_id, type, source, symbol, base, quote, contract, timeframe, side, quantity, filled_quantity, status, risk, fills, parameters, metadata, exec_correlation_id, exec_causation_id, final, timestamp)",
