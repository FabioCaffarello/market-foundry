...
func (h *Hub) Admit(conn ports.DeliveryConn) ports.DeliverySession {
	...
	return &SessionHandle{engine: h.engine, router: h.router, pid: pid, id: id}
}
...
// Close unregisters the session from the router and poisons the session
// actor (which closes the connection on stop).
