...
func TestPositionExposureEvaluatorActor_Long_Approved(t *testing.T) {
	...
	// Verify decision context carried through.
...
func TestPositionExposureEvaluatorActor_SequentialStrategies_Independent(t *testing.T) {
	...
	e.Send(evalPID, strategyResolvedMessage{
		...
		StrategyConfidence: "0.8000", DecisionSeverity: "high", Timeframe: 60, Timestamp: base,
	})
	// Flat → Approved.
	e.Send(evalPID, strategyResolvedMessage{
		Symbol: "btcusdt", StrategyType: "mean_reversion_entry", StrategyDirection: "flat",
		StrategyConfidence: "0.0000", DecisionSeverity: "none", Timeframe: 60, Timestamp: base.Add(time.Minute),
	})
	...
	// Verify decision severity isolation between sequential events.
	if r0.Strategies[0].DecisionSeverity != "high" {
		t.Errorf("first decision severity: want high, got %s", r0.Strategies[0].DecisionSeverity)
	}
	if r1.Strategies[0].DecisionSeverity != "none" {
		t.Errorf("second decision severity: want none, got %s", r1.Strategies[0].DecisionSeverity)
	}
}

func TestPositionExposureEvaluatorActor_FanOut_IncludesDecisionSeverity(t *testing.T) {
	e := newTestEngine(t)
