...
// TrendFollowingEntryResolverActor owns a TrendFollowingEntryResolver and publishes finalized strategies.
// It receives decisionEvaluatedMessage from the decision evaluator via local fan-out.
type TrendFollowingEntryResolverActor struct {
	cfg      StrategyResolverConfig
	logger   *slog.Logger
	resolver *appstrategy.TrendFollowingEntryResolver
}

func NewTrendFollowingEntryResolverActor(cfg StrategyResolverConfig) actor.Producer {
	return func() actor.Receiver {
		return &TrendFollowingEntryResolverActor{cfg: cfg}
	}
}

func (a *TrendFollowingEntryResolverActor) Receive(c *actor.Context) {
	if a.logger == nil {
		a.logger = slog.Default().With(
			"actor", "trend-following-entry-resolver",
			"source", a.cfg.Source,
			"symbol", a.cfg.Symbol,
	...
	case actor.Stopped:
		a.logger.Info("trend following entry resolver stopped")
...
func (a *TrendFollowingEntryResolverActor) onDecisionEvaluated(c *actor.Context, msg decisionEvaluatedMessage) {
	...
	a.logger.Info("trend following entry strategy resolved",
