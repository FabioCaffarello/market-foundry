...
// TestGolden_GeneratorMatchesCommittedFixture validates that the
// synthetic event generator currently produces output equal to the
// committed fixture. If a developer changes the generator (seed,
// distribution constants, included event types) without running
// `make golden-regen`, this test catches the drift before review.
//
// This test also exists so golangci-lint's `unused` check sees a
// non-build-tagged caller of generateSyntheticReplayCycleEvents;
// the only other caller lives behind the `goldenregen` build tag
// and is invisible under the default lint configuration.
func TestGolden_GeneratorMatchesCommittedFixture(t *testing.T) {
	...
	if _, err := r.WriteTo(&generated); err != nil {
		t.Fatalf("WriteTo: %v", err)
	}

	committed, err := os.ReadFile(goldenReplayCyclePath)
	if err != nil {
		t.Fatalf("read fixture %s: %v", goldenReplayCyclePath, err)
	}
	...
	if !bytes.Equal(generated.Bytes(), committed) {
		t.Fatalf(
			"generator output diverges from committed fixture\n"+
				"  fixture path: %s\n"+
				"  generated bytes (%d): %s\n"+
				"  committed bytes (%d): %s\n"+
				"\nIf the generator change was intentional, regenerate with:\n"+
				"  make golden-regen SCOPE=replay-cycle\n"+
				"and commit the new fixture alongside the generator change.",
			goldenReplayCyclePath,
			generated.Len(), truncate(generated.Bytes(), 256),
			len(committed), truncate(committed, 256),
		)
	}
}
