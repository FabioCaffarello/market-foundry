...
func validIntent() execution.ExecutionIntent {
...
func TestExecutionIntent_PartitionKey(t *testing.T) {
	ei := execution.ExecutionIntent{Source: "binancef", Symbol: "btcusdt", Timeframe: 60}
...
func TestExecutionIntent_MultiSymbol_PartitionKeyIsolation(t *testing.T) {
	...
	for _, sym := range symbols {
		for _, tf := range timeframes {
			ei := execution.ExecutionIntent{Source: "binancef", Symbol: sym, Timeframe: tf}
			key := ei.PartitionKey()
