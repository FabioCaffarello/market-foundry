...
// StrategyHandler is called for each decoded strategy resolved event.
type StrategyHandler func(strategy.StrategyResolvedEvent)

// StrategyConsumer is a durable JetStream consumer for strategy events.
type StrategyConsumer struct {
	url      string
	spec     ConsumerSpec
...
func NewStrategyConsumer(url string, spec ConsumerSpec, registry StrategyRegistry, handler StrategyHandler, logger *slog.Logger) *StrategyConsumer {
...
func (c *StrategyConsumer) Start() error {
...
func (c *StrategyConsumer) onMessage(msg jetstream.Msg) {
	env, prob := decodeEvent[strategy.StrategyResolvedEvent](c.spec.Event, msg.Data())
	if prob != nil {
...
func (c *StrategyConsumer) terminateOrNak(msg jetstream.Msg, prob *problem.Problem) {
...
func (c *StrategyConsumer) Close() error {
	if c.consumer != nil {
		c.consumer.Stop()
	}
	if c.nc != nil {
