...
// FillHandler is called for each decoded venue order filled event.
type FillHandler func(execution.VenueOrderFilledEvent)

// FillConsumer is a durable JetStream consumer for venue order fill events.
...
func NewFillConsumer(url string, spec natskit.ConsumerSpec, registry Registry, handler FillHandler, logger *slog.Logger) *FillConsumer {
	return &FillConsumer{
		url:      url,
		spec:     spec,
		registry: registry,
...
func (c *FillConsumer) Start() error {
	...
	if _, err := js.CreateOrUpdateStream(ctx, c.registry.VenueMarketOrderFilled.Stream.Config()); err != nil {
		nc.Close()
		return fmt.Errorf("ensure fill events stream: %w", err)
	}
	...
	if err != nil {
		nc.Close()
		return fmt.Errorf("create durable fill consumer: %w", err)
	}

	consumeCtx, err := cons.Consume(c.onMessage)
	if err != nil {
		nc.Close()
		return fmt.Errorf("start fill consume: %w", err)
	}
...
func (c *FillConsumer) onMessage(msg jetstream.Msg) {
	...
	if meta != nil && c.spec.MaxDeliver > 0 && meta.NumDelivered >= uint64(c.spec.MaxDeliver) {
		c.logger.Error("fill event at max delivery — will be terminated by NATS after this attempt",
			"subject", msg.Subject(),
			"num_delivered", meta.NumDelivered,
			"max_deliver", c.spec.MaxDeliver,
			"stream_seq", meta.Sequence.Stream,
	...
	env, prob := natskit.DecodeEvent[execution.VenueOrderFilledEvent](c.spec.Event, msg.Data())
	if prob != nil {
		c.logger.Error("decode fill event",
			"error", prob.Message,
			"code", prob.Code,
			"subject", msg.Subject(),
		)
...
func (c *FillConsumer) terminateOrNak(msg jetstream.Msg, prob *problem.Problem) {
	if prob.Code == problem.InvalidArgument {
		...
		if err := msg.Term(); err != nil {
			c.logger.Error("term fill event", "error", err)
		}
		return
	}
	c.nakked.Add(1)
...
func (c *FillConsumer) Stats() (delivered, redelivered, terminated, nakked int64) {
