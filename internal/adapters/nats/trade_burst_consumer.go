...
// TradeBurstHandler is called for each decoded evidence trade burst event.
type TradeBurstHandler func(evidence.TradeBurstSampledEvent)

// TradeBurstConsumer is a durable JetStream consumer for evidence trade burst events.
...
func NewTradeBurstConsumer(url string, spec ConsumerSpec, registry EvidenceRegistry, handler TradeBurstHandler, logger *slog.Logger) *TradeBurstConsumer {
...
func (c *TradeBurstConsumer) onMessage(msg jetstream.Msg) {
	env, prob := decodeEvent[evidence.TradeBurstSampledEvent](c.registry.TradeBurstSampled, msg.Data())
	if prob != nil {
