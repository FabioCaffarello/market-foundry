...
// VolumeHandler is called for each decoded evidence volume event.
type VolumeHandler func(evidence.VolumeSampledEvent)

// VolumeConsumer is a durable JetStream consumer for evidence volume events.
...
func NewVolumeConsumer(url string, spec ConsumerSpec, registry EvidenceRegistry, handler VolumeHandler, logger *slog.Logger) *VolumeConsumer {
...
func (c *VolumeConsumer) onMessage(msg jetstream.Msg) {
	env, prob := decodeEvent[evidence.VolumeSampledEvent](c.registry.VolumeSampled, msg.Data())
	if prob != nil {
