...
// Handler is called for each decoded decision evaluated event.
type Handler func(decision.DecisionEvaluatedEvent)

// Consumer is a durable JetStream consumer for decision events.
...
func (c *Consumer) onMessage(msg jetstream.Msg) {
	env, prob := natskit.DecodeEvent[decision.DecisionEvaluatedEvent](c.spec.Event, msg.Data())
	if prob != nil {
