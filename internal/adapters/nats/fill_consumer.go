...
func NewFillConsumer(url string, spec ConsumerSpec, registry ExecutionRegistry, handler FillHandler, logger *slog.Logger) *FillConsumer {
	return &FillConsumer{
		url:      url,
		spec:     spec,
		registry: registry,
...
func (c *FillConsumer) onMessage(msg jetstream.Msg) {
	...
	env, prob := decodeEvent[execution.VenueOrderFilledEvent](c.spec.Event, msg.Data())
	if prob != nil {
