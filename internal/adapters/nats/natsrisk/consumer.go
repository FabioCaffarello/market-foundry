...
// Handler is called for each decoded risk assessed event.
type Handler func(risk.RiskAssessedEvent)

// Consumer is a durable JetStream consumer for risk events.
...
func (c *Consumer) onMessage(msg jetstream.Msg) {
	...
	env, prob := natskit.DecodeEvent[risk.RiskAssessedEvent](c.spec.Event, msg.Data())
	if prob != nil {
